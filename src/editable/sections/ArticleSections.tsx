  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-[2.5rem] border ${pal.border} ${pal.darkBg} p-7 text-white shadow-[0_24px_80px_rgba(0,0,0,0.5)] sm:p-10 lg:p-14`}>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {pagination.hasPrevPage ? <Link href={pageHref(page - 1)} className={`rounded-full border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black ${pal.panelText}`}>Previous</Link> : null}
          <span className={`rounded-full ${pal.darkBg} px-5 py-3 text-sm font-black text-white`}>Page {page} of {pagination.totalPages || 1}</span>
          {pagination.hasNextPage ? <Link href={pageHref(page + 1)} className={`rounded-full border ${pal.border} bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-black ${pal.panelText}`}>Next</Link> : null}
        </div>
      </section>
    </main>
  )
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-[2.5rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.5)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            <Link href="/article" className={`inline-flex items-center gap-2 rounded-full border ${pal.border} bg-[var(--slot4-panel-bg)] px-4 py-2 text-sm font-black ${pal.panelText}`}><ChevronLeft className="h-4 w-4" /> Articles</Link>
          ...
          <aside className={`min-w-0 rounded-[2rem] ${pal.darkBg} p-6 text-white`}>
            ...
            <Link href="/contact" className={`mt-6 inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-black text-[var(--slot4-on-accent)]`}>Contact <ArrowRight className="h-4 w-4" /></Link>
          </aside>
        </div>
      </section>
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-[2.25rem] border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.5)] sm:p-8 lg:p-10`}>
